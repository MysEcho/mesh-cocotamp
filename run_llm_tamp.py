...
from llm_tamp.co_model import CorrelationalObsModel
...
def post_process(state, plan, co_updater: CorrelationalObsModel,
    ...
    for i, (name, args) in enumerate(plan):
        ...
        if name == 'move_base':
            ...
            if ht0 is None:
                new_commands = [ht, CoDetect(robot, o, l, co_updater)]
            else:
                new_commands = [ht0, ht, CoDetect(robot, o, l, co_updater)]
            expecting_obs = True
        else:
...
def plan_commands(state: BeliefState, co_updater: CorrelationalObsModel,args, profile=True, verbose=True):
    ...
    with Profiler(field='cumtime', num=10 if profile else None):
        ...
        commands = post_process(state, plan, co_updater)
    saver.restore()
    disconnect()
    return commands

def apply_commands(state, commands, obj_gt, time_step=None, pause=False, **kwargs):
...
def save_results(exp_results, use_llm, use_correlation, counter=0):
    if (not use_llm) and (not use_correlation):
        with open("results/intermediate/baseline_%d.json"%counter, "w") as f:
            json.dump(exp_results, f, indent=4) 
    
    elif (not use_llm) and (use_correlation):
        with open("results/intermediate/correlation_no_toggle_%d.json"%counter, "w") as f:
            json.dump(exp_results, f, indent=4)
    
    elif (use_llm) and (not use_correlation):
        # Save results to a JSON file
        with open("results/intermediate/rank_llm_%d.json"%counter, "w") as f:
            json.dump(exp_results, f, indent=4)
    
    elif (use_llm) and (use_correlation):
        # Save results to a JSON file
        with open("results/intermediate/rank_co_llm_%d.json"%counter, "w") as f:
            json.dump(exp_results, f, indent=4)
...
def main(time_step: float=0.01):
    ...
    exp_results = {}
    for use_llm, use_correlation in [(False, True)]:
        for id in range(4):
            ...
            context = 'llm_tamp/dataset/context.json'
            co_updater = CorrelationalObsModel(target_objs, items, context,
                                            task.class_from_body,
                                            task.body_from_class,
            ...
            while True:
                ...
                with ClientSaver():
                    ...
                    commands = plan_commands(state, co_updater, args)
                ...
                # wait_for_user()
                start_time = time.perf_counter()
                apply_commands(state, commands, obj_gt, time_step=time_step)
            ...
            save_results(exp_results, use_llm, use_correlation, id)
            # wait_for_user()
            disconnect()
            time.sleep(2)

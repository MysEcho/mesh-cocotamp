...
def pddlstream_from_state(state: BeliefState, teleport=True) -> PDDLProblem:
    task = state.task
    robot = task.robot
    ...
    stream_map = {
        ...
        'plan-base-motion': from_fn(get_motion_gen(task, teleport=True)),
        'inverse-kinematics': from_gen_fn(get_ik_ir_gen(task, max_attempts=1000, learned=True, teleport=True)),
...
def main(time_step: float=0.01):
    ...
    if use_llm_updater:
        co_updater = LLMUpdater(target_objs=target_objs,
    ...
    default_base_limits = get_base_limits(robot) # ([-5.0, -5.0], [5.0, 5.0])
    # large_base_limits = ([-14.6, -5.4], [14.6, 5.4])
